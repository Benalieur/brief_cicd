...
for todo in todos:
    with st.expander(
        f"{todo['title']} - {'Complétée' if todo['completed'] else 'Non complétée'}"
    ):
        new_title = st.text_input(
            "Modifier le titre", todo["title"], key=f"title_{todo['id']}"
        )
        ...
        new_completed = st.checkbox(
            "Complétée", todo["completed"], key=f"comp_{todo['id']}"
        )
